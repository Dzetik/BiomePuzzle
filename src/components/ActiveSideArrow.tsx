...
/**
 * Треугольная стрелка, указывающая активную сторону плитки.
 *
 * Треугольник строится через border-трюк CSS/RN: нулевой размер View
 * с ненулевыми прозрачными боковыми границами и цветной нижней границей
 * даёт равнобедренный треугольник. Поворот в нужное направление реализован
 * через `transform: rotate`. Под основным треугольником рендерится
 * чуть бо́льший тёмный треугольник — имитация обводки для контраста.
 *
 * @param size      - длина основания треугольника в пикселях (по умолчанию 16)
 * @param color     - цвет стрелки (по умолчанию белый)
 * @param direction - сторона плитки, на которую указывает стрелка
 * @param style     - дополнительные стили для позиционирования контейнера
 */
